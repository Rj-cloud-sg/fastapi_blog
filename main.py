...
# template route for viewing specific user post

# user endpoint for creating a new user
@app.post(
    "/api/users",
...
async def create_user(user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    db.add(new_user) # doesn't need await because it's just adding the new user to the session, not executing a query
    await db.commit()
...
async def delete_user(user_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    await db.delete(user) # Does need await because it's executing a query to delete the user from the database
    await db.commit()
...
async def create_post(post: PostCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    db.add(new_post)
    await db.commit()
    await db.refresh(new_post, attribute_names=["author"]) # Telling refresh to also load the related author object for the new post, so that it can be included in the response
    return new_post
